...
function installkicad {
    ...
    if [[ "$VERSION_ID" == "22.04" || "$VERSION_ID" == "24.04" ]]; then
        echo "Adding KiCad-8 PPA for supported Ubuntu version..."
        sudo add-apt-repository -y ppa:kicad/kicad-8.0-releases
        sudo apt update
        sudo apt install -y --no-install-recommends kicad
    else
        echo "WARNING: KiCad PPA not available for Ubuntu $VERSION_ID"
        echo "Attempting to install KiCad from Ubuntu repositories..."

        sudo apt update || true
        sudo apt install -y kicad || \
            echo "KiCad installation skipped. Please install manually if required."
    fi

    echo "KiCad installation step completed."
}
